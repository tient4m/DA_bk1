...
export class OrderService {
  ...
  exportInvoiceToPDF(id: number) {
    const url = `${this.apiUrl}/invoices/${id}/export/pdf`;

    return this.http.get(url, { responseType: 'blob' as 'json' }).pipe(
      catchError((error) => {
        debugger
        console.error('Error exporting invoice to PDF', error);
        return throwError(() => new Error('Error exporting invoice to PDF'));
      })
    );
  }
  ...
  getOrdersByUserId(userId: number, page: number, limit: number): Observable<OrderResponse> {
    debugger
    const params = new HttpParams()
      .set('page', page.toString())
      .set('limit', limit.toString());
